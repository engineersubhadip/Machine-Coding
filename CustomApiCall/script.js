...
let customAPIFetch = function(data){
...
customAPIFetch(data).then(function(data){
    let finalResult = data.reduce(function(acc,curr){
        acc.push(curr.title);
        return acc;
    },[]);

    let totalRecords = finalResult.reduce(function(acc,curr){
        return acc + 1;
    },0);

    console.log("The list of Records are",finalResult);
